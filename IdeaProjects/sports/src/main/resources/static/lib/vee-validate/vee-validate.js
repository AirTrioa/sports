...
(function (global, factory) {
  ...
  var confirmedPassword = function (value, ref, validatingField) {
    var confirmedField = ref[0];
  ...
  var decimalRetain = function (value, params) {
    var retainField = document.querySelector(("input[name='" + params[0] + "']"));
    var retainValue = retainField.value;

    var decimals = retainValue ? (retainValue || '*') : '*';
    if (Array.isArray(value)) {
      return false;
    }
  ...
  var bigger = function (value, ref) {
    var refField = document.querySelector(("input[name='" + ref[0] + "']"));
    var refValue = refField.value;

    if( refValue == "" || value > parseInt(refValue)){
      return true
    }else {
      return false
    }
  ...
  var ext = function (files, extensions) {
    ...
    if(extensions.length==1){
      regex = new RegExp((".(" + (extensions[0].split(',').join('|')) + ")$"), 'i');
    }else{
      regex = new RegExp((".(" + (extensions.join('|')) + ")$"), 'i');
    }
    if(typeof files == "string"){
      return regex.test(files);
    }else {
  ...
  var image = function (files) {
    var regex = new RegExp((".(jpg|svg|jpeg|png|bmp|gif)$"), 'i');
    if(typeof files == "string"){
      return regex.test(files);
    }else {
      return files.every(function (file) { return regex.test(file.name); });
  ...
  var regexBy = function (value, ref,validatingField) {
    var confirmedField = ref[0];

    var field = confirmedField
      ? document.querySelector(("select[name='" + confirmedField + "']"))
      : document.querySelector(("select[name='" + validatingField + "_confirmation']"));
    ...
    var regex = field.options[index].getAttribute('data-rule');
  ...
  var statusValue = function (value, ref) {
    var type = ref[0];
    var regex;
  ...
  var valueDefault = function (value, ref, valueField) {
    var minfield = document.querySelector(("input[name='" + ref[0] + "']"));
    var maxfield = document.querySelector(("input[name='" + ref[1] + "']"));
    var minvalue = minfield.value;
    var maxvalue = maxfield.value;

    if((minvalue == "" || maxvalue == "") || (value >= parseFloat(minvalue) && value <= parseFloat(maxvalue))){
      return true;
    }else {
      return false;
    }
  };
  var lengthDefault = function (value, ref, valueField) {
    var minfield = document.querySelector(("input[name='" + ref[0] + "']"));
    var maxfield = document.querySelector(("input[name='" + ref[1] + "']"));
    var minvalue = minfield.value;
    var maxvalue = maxfield.value;

    if((minvalue == "" || maxvalue == "") || (value.length >= minvalue && value.length <= maxvalue)){
      return true;
    }else {
      return false;
    }
  ...
  var isUnique = function (value, ref) {
    if (value === undefined || value === null) {
      return true;
    }
    var validateId;

    var validateUrl = ref[0];
    var validateTable = ref[1];
    var validateField = ref[2];

    if(ref.length > 3){
      var idTargetField = ref[3];
      var field = document.querySelector(("input[name='" + idTargetField + "']"));
      validateId = field?field.value:null;
    }else{
      validateId = null;
    }
  ...
  var messages = {
    after: function (n, e) {
      return "请输入在" + e[0] + "之后的日期"
    }, alpha_dash: function (n) {
      return "格式错误，仅支持字母、数字、“-”“_”的组合"
    }, alpha_num: function (n) {
      return "格式错误，仅支持字母、数字"
    }, alpha_spaces: function (n) {
      return "格式错误，仅支持字母、空格."
    }, alpha: function (n) {
      return "格式错误，仅支持字母"
    }, before: function (n, e) {
      return "请输入在" + e[0] + "之前的日期"
    }, between: function (n, e) {
      return "请输入在" + e[0] + "和" + e[1] + "之间的值"
    }, confirmed: function (n, e) {
      return " " + n + " 和" + e[0] + "的值不同"
    }, confirmedPassword: function () {
      return "两次密码不一致"
    }, date_between: function (n, e) {
      return "请输入在" + e[0] + "和" + e[1] + "之间的日期"
    }, date_format: function (n, e) {
      return "请输入正确的日期格式"
    }, decimal: function (n, e) {
      void 0 === e && (e = ["*"]);
      var t = e[0];
      return "格式错误，仅支持小数位为" + ("*" === t ? "" : t) + " 的小数"
    }, decimalRetain: function (n) {
      return "请输入符合设置的位数的小数"
    }, bigger: function (n) {
      return "最大值必须大于最小值"
    }, longer: function (n) {
      return "最大长度必须大于最小长度"
      ...
      return "请上传宽度 " + e[0] + " 像素，长度" + e[1] + " 像素之内的文件"
    }, email: function (n) {
      return "请输入正确的邮箱格式"
    }, ext: function (n) {
      return "文件格式错误"
      ...
      return "请输入长度不大于"+ e[0] + "的值"
    }, max_value: function (n, e) {
      return "请输入小于"+ e[0] + "的值"
    }, mimes: function (n) {
      return "请上传正确的文件类型."
    }, min: function (n, e) {
      return "请输入长度大于"+ e[0] + "的值"
      ...
      var reg = e[0];
      if(reg.ruleName != undefined){
        return "请输入正确的"+ reg.ruleName +"格式"
      }
      return "请输入正确的格式"
    }, regexBy: function (n,e) {
      var confirmedField = e[0];
      var field = document.querySelector(("select[name='" + confirmedField + "']"));

      var index = field.selectedIndex;
      var text = field.options[index].innerText;

      return "请输入正确的"+ text +"格式"
    }, statusValue:function (n,e) {
      var type = e[0];
      var text;
      if( type == "single_value"){
        text = "数值"
      ...
      return "请输入正确的"+ text +"格式"
    }, valueDefault: function(n){
      return "必须介于最小值与最大值之间"
    }, lengthDefault: function(n){
      return "必须介于最小长度与最大长度之间"
    },required: function (n) {
      return "请输入"
    }, size: function (n, e) {
      if(e[0] >= 1000){
        return "文件必须小于 " + e[0]/1000 + " M"
      }
      return "文件必须小于 " + e[0] + " KB"
    }, url: function (n) {
      return "请输入正确的URL格式"
    },isUnique: function (n) {
      return "输入值重复"
  ...
  var messages$1 = {
    after: function (field, ref) {
      ...
      return ("请输入在" + target + "之后的日期");
    },
    before: function (field, ref) {
      var target = ref[0];
      var inclusion = ref[1];

      return ("请输入在" + target + "之前的日期");
    },
    date_between: function (field, ref) {
      var min = ref[0];
      var max = ref[1];

      return ("请输入在" + min + "和" + max + "之间的日期");
    },
