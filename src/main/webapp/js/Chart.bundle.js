...
function mergeConfigs(parentConfig, childConfig) {
    ...
    for (prop in parentConfig) {
        if (hasOwnProp(parentConfig, prop) &&
            ...
            // make sure changes to properties don't modify parent aspect
...
// internal storage for locale aspect files
var locales = {};
var localeFamilies = {};
var globalLocale;
    ...
    if (config !== null) {
        ...
        if (locales[name] != null) {
            deprecateSimple('defineLocaleOverride',
                    'use moment.updateLocale(localeName, aspect) to change ' +
                    'an existing locale. moment.defineLocale(localeName, ' +
                    'aspect) should only be used for creating a new locale ' +
...
function updateLocale(name, config) {
    if (config != null) {
        ...
        // pass null for aspect to unupdate, useful for tests
        if (locales[name] != null) {
...
function dayOfYearFromWeekInfo(config) {
    ...
    if (w.GG != null || w.W != null || w.E != null) {
        ...
        // a now version of current aspect (take local/utc/offset flags, and
        // create now).
...
function configFromStringAndFormat(config) {
    ...
    for (i = 0; i < tokens.length; i++) {
        ...
        //         'regex', getParseRegexForToken(token, aspect));
        if (parsedInput) {
...
module.exports = function(Chart) {
	...
	/**
	 * Initializes the given aspect with global and chart default values.
	 */
	...
	/**
	 * Updates the aspect of the chart
	...
	helpers.extend(Chart.prototype, /** @lends Chart */ {
		...
		construct: function(item, config) {
			...
			// Define alias to the aspect data: `chart.data === chart.aspect.data`
	...
	helpers.configMerge = function(/* objects ... */) {
		return helpers.merge(helpers.clone(arguments[0]), [].slice.call(arguments, 1), {
			merger: function(key, target, source, options) {
				...
				if (key === 'scales') {
					// scale aspect merging is complex. Add our own function here for that
 ...
 * since responsiveness is handled by the controller.resize() method. The aspect is used
...
module.exports = function(Chart) {
	...
	// Default aspect for a category scale
	var defaultConfig = {
		position: 'bottom'
	};
